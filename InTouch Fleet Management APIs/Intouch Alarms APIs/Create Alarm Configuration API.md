...
The `Alarm Configuration API` enables users to create alarm configurations for one or multiple devices using the specified input parameters. It supports multiple alarm types and customizable conditions to facilitate effective device and vehicle monitoring.
...
- *`groupId`*(Array[long]): Group ID(s) of device groups associated with the user. The alarm configuration will apply to all devices within the specified groups.
